...
img1 = uint8( interp2( X,Y,double(img),Xi,Yi,'nearest') );  % Wartość punktu interpolowanego jest określana przez wartość najbliższego punktu danych. Innymi słowy, interpolowana wartość to wartość najbliższego punktu w oryginalnym zbiorze danych.
img2 = uint8( interp2( X,Y,double(img),Xi,Yi,'linear') );  % Połączenie danych punktów linią prostą
img3 = uint8( interp2( X,Y,double(img),Xi,Yi,'spline') ); % używane są sklejane funkcje kawałkowe odpowiedniego stopnia, co oznacza, że interpolacja jest realizowana poprzez łączenie wielomianów danego stopnia na poszczególnych podprzedziałach danych.
img4 = uint8( interp2( X,Y,double(img),Xi,Yi,'cubic') ); % Interpolacja Hermite'a polega na sklejaniu funkcji kubicznych na poszczególnych podprzedziałach danych.
