...
h = load('h.dat'); % rzeczywista odpowiedz impulsowa kanalu
L = length(h); % liczba wag
figure; stem(h,'filled'); title('h(n)'); grid; pause

prbs = 2*round( rand(30*L,1) )-1; % pobudzenie kanalu
...
SNR=10; y = awgn(y,SNR); % dodanie szumu, tak aby otrzymac zadany SNR
% Im niższy snr tym większa długość sygnału pobudzenia potrzebna żeby błąd
% estymacji był niski
